...
public class Maelstrom : Monster
{
    ...
    public override void Activate(FountainOfObjectsGame game)
    {
        ...
        game.Player.Location = StayOnMap(new Location(game.Player.Location.Row + 1, game.Player.Location.Column - 2), game.Map.Rows, game.Map.Columns);
        Location = StayOnMap(new Location(Location.Row + 2, Location.Column + 2), game.Map.Rows, game.Map.Columns);
    }

    private Location StayOnMap(Location location, int totalRows, int totalColumns)
    {
...
public class HelpCommand : ICommand
{
    public void Execute(FountainOfObjectsGame game)
    {
        ...
        ConsoleHelper.WriteLine("   Displays help information.", ConsoleColor.Gray);
        ...
        ConsoleHelper.WriteLine("   Moves one space up directly north from where the player is located, as long as it is still in the map.", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("move south", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("   Moves one space up directly south from where the player is located, as long as it is still in the map.", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("move east", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("   Moves one space up directly east from where the player is located, as long as it is still in the map.", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("move west", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("   Moves one space up directly west from where the player is located, as long as it is still in the map.", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("shoot north", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("   Shoots one arrow in the north direction from where the player is located, as long as the player still has arrows available.", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("shoot south", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("   Shoots one arrow in the south direction from where the player is located, as long as the player still has arrows available.", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("shoot east", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("   Shoots one arrow in the east direction from where the player is located, as long as the player still has arrows available.", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("shoot west", ConsoleColor.Gray);
        ConsoleHelper.WriteLine("   Shoots one arrow in the west direction from where the player is located, as long as the player still has arrows available.", ConsoleColor.Gray);
    }
}
...
public class DraftFromPitSense : ISense
{
